...
# Uyarıları bastırmak için (isteğe bağlı ama önerilir)
...
def load_data_for_test(data_path):
    ...
    _, X_test, _, y_test = train_test_split(X, y, test_size=0.3, random_state=42, stratify=df['binary_label'])
    
    y_test_corrected = np.array(["benign" if label == "good" else label for label in y_test])
    
    print(f"Test data loaded. Total samples: {len(X_test)}")
    return X_test, y_test_corrected

# === BU FONKSİYON GÜNCELLENDİ ===
def run_hybrid_prediction(X_test, binary_model, dqn_multiclass_model, multiclass_label_map, binary_model_type):
    """İki aşamalı tahmin boru hattını çalıştırır."""
    final_predictions = []
    dqn_device = dqn_multiclass_model.device
    
    for i in range(len(X_test)):
        # --- Aşama 1: İkili Sınıflandırma ---
        if binary_model_type == 'ml':
            # ML modelleri için: Tek satırlık bir DataFrame vererek sütun adlarını koru.
            # .iloc[[i]] (çift köşeli parantez) bir DataFrame döndürür.
            feature_row_df = X_test.iloc[[i]]
            binary_pred = binary_model.predict(feature_row_df)[0]
        else: # dqn
            # DQN modelleri için: NumPy dizisi ver.
            feature_values_np = X_test.iloc[i].values
            binary_pred, _ = binary_model.predict(feature_values_np, deterministic=True)

        # --- Aşama 2: Çok Sınıflı Sınıflandırma ---
        if binary_pred == 0:
            final_predictions.append("benign")
        else:
            # Multiclass DQN her zaman NumPy dizisi kullanır.
            feature_values_np = X_test.iloc[i].values
...
def main(binary_choice):
    # --- Dosya Yolları ---
    # .zip uzantılarını dosya adlarından kaldırdım, stable-baselines3 bunu kendi hallediyor.
    ...
    if binary_choice not in model_paths:
        raise ValueError(f"Invalid choice '{binary_choice}'. Must be one of {list(model_paths.keys())}")
    ...
    try:
        ...
        print("Please make sure you have trained all the necessary models first.")
        return
    ...
    final_predictions = run_hybrid_prediction(X_test, binary_model, dqn_multiclass_model, multiclass_label_map, binary_model_type)
    
    evaluate_and_plot(y_test_corrected, final_predictions, f"End-to-End Performance (Binary: {binary_choice.upper()})")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate the hybrid URL classification system.")
    parser.add_argument(
        '--binary_model', 
        type=str, 
        required=True, 
