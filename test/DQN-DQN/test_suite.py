...
# 🔄 Model yükleme
try:
    dqn_binary_model = DQN.load("binary-pkl/binary_dqn_model.zip")
...
for i in range(len(X_test)):
    ...
    try:
        dqn_binary_prediction = dqn_binary_model.predict(feature)[0]
    except Exception as e:
        print(f"DQN Binary tahmin hatası: {e}")
        continue

    binary_predictions.append(dqn_binary_prediction)

    if dqn_binary_prediction == 0:
