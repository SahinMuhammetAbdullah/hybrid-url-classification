...
class URLTypeClassificationEnv(gym.Env):
    """
    URL Tür Sınıflandırması için özel Gymnasium ortamı.
    """
    ...
    def reset_indices(self):
        """Örneklerin sırasını karıştırır ve adımı sıfırlar."""
    ...
    def reset(self, seed=None, options=None):
        ...
        initial_obs = self.features[self.shuffled_indices[self.current_step]]
        # Yeni gymnasium API'sine uygun: obs, info
        return initial_obs, {}
    ...
    def step(self, action):
        ...
        if action == true_label:
            ...
            reward = -(1.0 / self.class_weights[true_label])

        self.current_step = (self.current_step + 1) % len(self.features)
        done = self.current_step == 0

        next_idx = self.shuffled_indices[self.current_step]
        next_obs = self.features[next_idx]

        if done:
            episode_reward_bonus = self.calculate_multimetric_reward()
            reward += episode_reward_bonus

            self.predictions = []
            self.true_labels_log = []
            self.transition_log = []

        # Yeni gymnasium API'sine uygun: obs, reward, terminated, truncated, info
        # Monitor ve DummyVecEnv için (terminated, truncated = done, False)
        return next_obs, reward, done, False, {}

    def compute_class_weights(
        self, labels, min_weight=0, max_weight=None, normalize=True
    ):
        ...
        if normalize:
            ...
            if total_weight > 0:
                ...
                for cls in weights:
                    weights[cls] = 1.0 / len(unique_classes)
    ...
    def calculate_multimetric_reward(self):
        if len(set(self.true_labels_log)) > 1:
            precision = precision_score(
                self.true_labels_log, self.predictions, average="macro", zero_division=0
            )
            recall = recall_score(
                self.true_labels_log, self.predictions, average="macro", zero_division=0
            )
